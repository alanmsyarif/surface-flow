"""Geometry Nodes builder for the Surface Flow MVP.
...
def build_surface_flow_group(force_rebuild: bool = True):
    material = ensure_flow_material()
    ...
    if tree is None:
        ...
        clear_nodes(tree)
        # Rebuilding the interface is easier and deterministic for this R&D stage.
    ...
    link(tree, group_in, ("Gravity",), g_norm, ("Vector",), to_fallback=0)

    g_neg = new_node(tree, "ShaderNodeVectorMath", "Up Direction", -1380, 500)
    g_neg.operation = 'SCALE'
    link(tree, g_norm, ("Vector",), g_neg, ("Vector",), from_fallback=0, to_fallback=0)
    ...
    link(tree, pos_src, ("Position",), height_dot, ("Vector",), to_fallback=0)
    link(tree, g_neg, ("Vector",), height_dot, ("Vector",), from_fallback=0, to_fallback=1)
    ...
    link(tree, group_in, ("Source Start",), lower, ("Value",), to_fallback=0)
    link(tree, group_in, ("Source Softness",), lower, ("Value",), to_fallback=1)

    upper = new_node(tree, "ShaderNodeMath", "Source Upper", -900, -70)
    upper.operation = 'ADD'
    link(tree, group_in, ("Source Start",), upper, ("Value",), to_fallback=0)
    link(tree, group_in, ("Source Softness",), upper, ("Value",), to_fallback=1)
    ...
    link(tree, source_mask, ("Result",), density, ("Value",), to_fallback=0)
    link(tree, group_in, ("Seed Density",), density, ("Value",), to_fallback=1)
    ...
    link(tree, g_norm, ("Vector",), dot_gn, ("Vector",), from_fallback=0, to_fallback=0)
    link(tree, sample_normal, ("Value",), dot_gn, ("Vector",), to_fallback=1)

    n_scaled = new_node(tree, "ShaderNodeVectorMath", "Normal Component", 1800, 600)
    n_scaled.operation = 'SCALE'
    link(tree, sample_normal, ("Value",), n_scaled, ("Vector",), to_fallback=0)
    ...
    link(tree, g_norm, ("Vector",), tangent, ("Vector",), from_fallback=0, to_fallback=0)
    link(tree, n_scaled, ("Vector",), tangent, ("Vector",), from_fallback=0, to_fallback=1)

    tangent_norm = new_node(tree, "ShaderNodeVectorMath", "Normalize Tangent", 2210, 580)
    tangent_norm.operation = 'NORMALIZE'
    link(tree, tangent, ("Vector",), tangent_norm, ("Vector",), to_fallback=0)
    ...
    link(tree, group_in, ("Seed",), seed_add, ("Value",), to_fallback=0)
    link(tree, repeat_in, ("Iteration",), seed_add, ("Value",), to_fallback=1)
    try:
    ...
    link(tree, rand, ("Value",), noise_dot, ("Vector",), to_fallback=0)
    link(tree, sample_normal, ("Value",), noise_dot, ("Vector",), to_fallback=1)

    noise_n = new_node(tree, "ShaderNodeVectorMath", "Noise Normal Component", 2000, 330)
    noise_n.operation = 'SCALE'
    link(tree, sample_normal, ("Value",), noise_n, ("Vector",), to_fallback=0)
    ...
    link(tree, rand, ("Value",), noise_tan, ("Vector",), to_fallback=0)
    link(tree, noise_n, ("Vector",), noise_tan, ("Vector",), from_fallback=0, to_fallback=1)

    noise_scale = new_node(tree, "ShaderNodeVectorMath", "Noise Strength", 2400, 330)
    noise_scale.operation = 'SCALE'
    link(tree, noise_tan, ("Vector",), noise_scale, ("Vector",), to_fallback=0)
    ...
    link(tree, tangent_norm, ("Vector",), direction_add, ("Vector",), to_fallback=0)
    link(tree, noise_scale, ("Vector",), direction_add, ("Vector",), to_fallback=1)

    direction_norm = new_node(tree, "ShaderNodeVectorMath", "Normalize Flow", 2640, 560)
    direction_norm.operation = 'NORMALIZE'
    link(tree, direction_add, ("Vector",), direction_norm, ("Vector",), to_fallback=0)

    step_vec = new_node(tree, "ShaderNodeVectorMath", "Step Vector", 2850, 540)
    step_vec.operation = 'SCALE'
    link(tree, direction_norm, ("Vector",), step_vec, ("Vector",), to_fallback=0)
    ...
    link(tree, repeat_in, ("Iteration",), iter_plus_one, ("Value",), to_fallback=0)
    set_input(iter_plus_one, ("Value",), 1.0, fallback=1)
    ...
    link(tree, normal_attr, ("Attribute",), normal_offset, ("Vector",), to_fallback=0)
